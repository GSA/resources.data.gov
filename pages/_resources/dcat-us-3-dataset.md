...
details: >+
  ...
  For the full technical schema reference, see the [DCAT-US 3.0 Dataset schema reference](https://resources.data.gov/standards/catalog/dcat-us-3/dataset/).
  ...
  **Required fields in v3.0:** `title`, `description`, `contactPoint`, `identifier`


  
  **Note on `publisher`:** As of May 27, 2026, `publisher` has been changed from Mandatory to Optional in the v3.0 schema. Agencies are still strongly encouraged to provide it.
  ...
  <td>Optional</td>
  <td>object</td>
  <td>The publishing entity and optionally its parent organization(s). References the <a href="../dcat-us-3-supporting-classes/">Organization class</a>. At a minimum, provide <code>name</code>. Optionally add <code>prefLabel</code> (preferred legal name), <code>altLabel</code> (acronym), and <code>subOrganizationOf</code> (array of parent organizations). Strongly encouraged even though no longer required by the schema.</td>
  </tr>
  <tr id="contactPoint">
  ...
  <td>Optional</td>
  <td>string (ISO 8601)</td>
  <td>Date of formal release. If the exact publication date is unknown, use the date it was first referenced in the catalog.</td>
  </tr>
  <tr id="created">
  <td><code>created</code></td>
  <td>Optional</td>
  <td>string (ISO 8601)</td>
  <td>The date the dataset was first created. Distinct from <code>issued</code> (the public release date) — use this when the creation date and publication date differ.</td>
  </tr>
  <tr id="language">
  ...
  <tr id="otherIdentifier">
  <td><code>otherIdentifier</code></td>
  <td>Optional</td>
  <td>array of Identifier objects</td>
  <td>Additional identifiers for the dataset beyond the main identifier, such as a DOI or other persistent ID. References the <a href="../dcat-us-3-supporting-classes/">Identifier class</a>.</td>
  </tr>
  <tr id="status">
  <td><code>status</code></td>
  <td>Optional</td>
  <td>Concept object</td>
  <td>Lifecycle status of the dataset, such as completed, deprecated, under development, or withdrawn. References the <a href="../dcat-us-3-supporting-classes/">Concept class</a>.</td>
  </tr>
  <tr id="image">
  <td><code>image</code></td>
  <td>Optional</td>
  <td>string (IRI)</td>
  <td>A thumbnail image illustrating the dataset. Especially useful for visual data such as maps, photos, or video. Provide as a URL.</td>
  </tr>
  <tr id="page">
  <td><code>page</code></td>
  <td>Optional</td>
  <td>array of Document objects</td>
  <td>Pages or documents about this dataset. Use for related documentation, reports, or publications that describe the dataset. References the <a href="../dcat-us-3-supporting-classes/">Document class</a>.</td>
  </tr>
  </tbody>
  </table>
  ...
  <td>Recommended</td>
  <td>array of Distribution objects</td>
  <td>A container for one or more Distribution objects describing how the dataset can be accessed or downloaded. Can be omitted when no distribution is available yet. See <a href="../dcat-us-3-distribution/">Distribution fields</a>. In v3.0, <code>license</code> should be included on each Distribution object rather than at the Dataset level. If all distributions share the same license, add it to each one.</td>
  </tr>
  <tr id="sample">
  <td><code>sample</code></td>
  <td>Optional</td>
  <td>array of Distribution objects</td>
  <td>Sample distributions for the dataset — for example, a small subset of the data provided as a preview.</td>
  </tr>
  <tr id="metadataDistribution">
  <td><code>metadataDistribution</code></td>
  <td>Optional</td>
  <td>array of Distribution objects</td>
  <td>A distribution of the original metadata document this dataset was derived from — for example, an ISO 19115 XML record or a FGDC metadata file.</td>
  </tr>
  </tbody>
  </table>
  ...
  <td>Organizations holding rights on the dataset. References the <a href="../dcat-us-3-supporting-classes/">Organization class</a>.</td>
  </tr>
  <tr id="license">
  ...
  <tr id="isReferencedBy">
  <td><code>isReferencedBy</code></td>
  <td>Optional</td>
  <td>array of strings (IRI)</td>
  <td>Links to related resources, such as publications, that reference, cite, or otherwise point to the dataset. Example: <code>["https://example.gov/publications/climate-trends-2024"]</code></td>
  </tr>
  </tbody>
  </table>
  ...
  <tr id="spatialResolutionInMeters">
  ...
  <tr id="temporalResolution">
  <td><code>temporalResolution</code></td>
  <td>Optional</td>
  <td>string (ISO 8601 duration)</td>
  <td>Minimum time period resolvable in the dataset. Example: <code>"P1D"</code> for daily resolution. New in v3.0.</td>
  </tr>
  <tr id="subject">
  <td><code>subject</code></td>
  <td>Optional</td>
  <td>array of Concept objects</td>
  <td>Primary subjects for the dataset, usually narrower than broad theme categories. References the <a href="../dcat-us-3-supporting-classes/">Concept class</a>.</td>
  </tr>
  <tr id="category">
  <td><code>category</code></td>
  <td>Optional</td>
  <td>array of Concept objects</td>
  <td>High-level categories for the dataset. Similar to <code>theme</code> but intended for broader groupings. References the <a href="../dcat-us-3-supporting-classes/">Concept class</a>.</td>
  </tr>
  </tbody>
  </table>
  ...
  <td>Optional</td>
  <td>array of Dataset objects</td>
  <td>Related datasets that are part of this dataset. Use this to express a collection or compound dataset relationship.</td>
  </tr>
  <tr id="first">
  <td><code>first</code></td>
  <td>Optional</td>
  <td>Dataset object</td>
  <td>The first item in a sequence this dataset belongs to.</td>
  </tr>
  <tr id="relation">
  <td><code>relation</code></td>
  <td>Optional</td>
  <td>array of strings (IRI)</td>
  <td>Links to related resources when the relationship is not otherwise specified.</td>
  </tr>
  <tr id="qualifiedRelation">
  <td><code>qualifiedRelation</code></td>
  <td>Optional</td>
  <td>array of Relationship objects</td>
  <td>Detailed relationship between the dataset and another resource, including the named role of that relationship. References the <a href="../dcat-us-3-supporting-classes/">Relationship class</a>.</td>
  </tr>
  </tbody>
  </table>
  ...
  <td>Optional</td>
  <td>array of Activity objects</td>
  <td>Activities that used this dataset. References the <a href="../dcat-us-3-supporting-classes/">Activity class</a>.</td>
  </tr>
  <tr id="wasAttributedTo">
  <td><code>wasAttributedTo</code></td>
  <td>Optional</td>
  <td>array of Agent objects</td>
  <td>Agents attributed to this dataset. References the <a href="../dcat-us-3-supporting-classes/">Agent class</a>.</td>
  </tr>
  <tr id="contributor">
  <td><code>contributor</code></td>
  <td>Optional</td>
  <td>array of Agent objects</td>
  <td>Agents who contributed to the dataset. References the <a href="../dcat-us-3-supporting-classes/">Agent class</a>.</td>
  </tr>
  <tr id="qualifiedAttribution">
  <td><code>qualifiedAttribution</code></td>
  <td>Optional</td>
  <td>array of Attribution objects</td>
  <td>Agents with specific named responsibilities for the dataset. References the <a href="../dcat-us-3-supporting-classes/">Attribution class</a>.</td>
  </tr>
  <tr id="hasQualityMeasurement">
  <td><code>hasQualityMeasurement</code></td>
  <td>Optional</td>
  <td>array of QualityMeasurement objects</td>
  <td>Quality measurements associated with the dataset — for example, completeness, accuracy, or timeliness. References the <a href="../dcat-us-3-supporting-classes/">QualityMeasurement class</a>.</td>
  </tr>
  </tbody>
  </table>
  ...
  <tbody>
  <tr id="change-publisher">
  <td><code>publisher</code></td>
  <td>Changed from Mandatory to Optional (May 2026)</td>
  <td>Still strongly encouraged. Provide at minimum an <code>Organization</code> object with <code>name</code>.</td>
  </tr>
  <tr id="change-accessLevel">
  <td><code>accessLevel</code></td>
  ...
  | Date | Change |
  |---|---|
  | 2026-05-27 | Added missing fields: `status`, `supportedSchema`, `first`, `qualifiedRelation`, `metadataDistribution`, `isReferencedBy`, `created`, `contributor`, `hasPart`, `relation`, `subject`, `category`, `page`, `qualifiedAttribution`, `wasAttributedTo`, `wasUsedBy`, `image`, `otherIdentifier`. Updated `publisher` requirement level from Mandatory to Optional per schema changelog. Added deep-link anchors to all table rows. Added link to full technical schema reference. |
