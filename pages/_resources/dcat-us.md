...
[accessLevel](#accessLevel)                                | Public Access Level       | The degree to which this dataset **could** be made publicly-available, *regardless of whether it has been made available*. Choices: public, restricted public, or non-public. | Always
...
[rights](#rights)                                          | Rights                    | This may include information regarding access or restrictions based on privacy, security, or other policies. Text, 255 characters. | If-Applicable
...
[describedBy](#dataset-describedBy)                        | Data Dictionary           | URL to the data dictionary for the dataset. | No
[describedByType](#dataset-describedByType)                | Data Dictionary Type      | The machine-readable file format (IANA Media Type) of the dataset's Data Dictionary (`describedBy`). | No
...
[landingPage](#landingPage)                                | Homepage URL              | A human-friendly hub or landing page for all resources tied to the dataset. | No
...
[describedBy](#distribution-describedBy)        | Data Dictionary       | URL to the data dictionary for the distribution found at the `downloadURL`. | No
[describedByType](#distribution-describedByType)| Data Dictionary Type  | The machine-readable file format (IANA Media Type) of the distribution's `describedBy` URL. | No
...
[mediaType](#distribution-mediaType)            | Media Type            | The machine-readable file format (IANA Media Type) of the distribution's `downloadURL`. | If-Applicable
...
**Example** | The Office of the Solicitor (86) at the Department of the Interior (010) would be: `{"bureauCode":["010:86"]}`. If a second bureau was also responsible: `{"bureauCode":["010:86","010:04"]}`.
...
**Usage Notes** | This should be included with `hasEmail` as part of a record's `contactPoint`.
**Example** | `{"fn": "Jane Doe"}`
...
**Usage Notes** | This is used to identify the media type of the URL used for the dataset's `describedBy` field. This should be specified if `describedBy` is not an HTML webpage.
...
**Example** | `{"description":"This dataset contains a list of vegetables, including nutrition information and seasonality."}`
...
**Required** | No
**Accepted Values** | String (URL)
**Usage Notes** | This is used to specify a data dictionary or schema that defines fields or column headings in the distribution.
...
**Required** | No
**Accepted Values** | String ([IANA Media Type](http://www.iana.org/assignments/media-types))
**Usage Notes** | This is used to identify the media type of the URL used for the distribution's `describedBy` field. This is especially important if `describedBy` is a machine readable file.
...
**Required** | No
**Accepted Values** | String
**Usage Notes** | This should be a human-readable description of the file format of the dataset. Note that `API` should always be used to distinguish web APIs.
**Example** | `{"format":"CSV"}`
...
**Required** | Yes, if the file is available for public download.
**Accepted Values** | String ([IANA Media Type](http://www.iana.org/assignments/media-types))
**Usage Notes** | This must describe the exact files available at `downloadURL` using a media type. For common Microsoft Office files, see [Office Open XML MIME types](https://support.microsoft.com/en-us/office/open-xml-formats-and-file-name-extensions-5200d93c-3449-4380-8e11-31ef14555b18).
...
**Required** | Yes, always
**Accepted Values** | String
**Usage Notes** | This field allows third parties to maintain a consistent record for datasets even if title or URLs are updated. Agencies may integrate an existing system for maintaining unique identifiers. Each identifier **must** be unique across the agency's catalog and remain fixed. It is **highly recommended** that a [URI (preferably an HTTP URL)](http://tools.ietf.org/html/rfc3986#section-1.1.3) be used to provide a globally unique identifier.
...
**Required** | Yes, always
**Accepted Values** | ISO 8601 Date
**Usage Notes** | Dates should be [ISO 8601](http://en.wikipedia.org/wiki/ISO_8601) of highest resolution. If there is a need to reflect that the dataset is continually updated, ISO 8601 formatting can account for this [with repeating intervals](http://en.wikipedia.org/wiki/ISO_8601#Time_intervals). For instance, `R/P1D` for daily, `R/P2W` for every two weeks, and `R/PT5M` for every five minutes.
...
**Example** | `{"references":["http://www.agency.gov/legumes/legumes_data_documentation.html"]}`
...
**Accepted Values** | String
**Usage Notes** | This may include information regarding access or restrictions based on privacy, security, or other policies. This should also serve as an explanation for the selected "accessLevel" including instructions for how to access a restricted file, if applicable. If the dataset can be made available through a website indirectly, use `accessURL` for the URL that provides such access.
...
**Usage Notes** | This field supports either Strings or GeoJSON Objects. As a string, this field should contain one of the following: (1) a bounding coordinate box in the order of minimum longitude, minimum latitude, maximum longitude, maximum latitude; (2) a latitude/longitude pair; (3) a geographic feature from the [GeoNames database](http://www.geonames.org). GeoJSON Polygon and Point types are also supported.
**Example** | `{"spatial": "137.5488,3.8128,163.3647,10.2284"}` or `{"spatial":"Lincoln, Nebraska"}`
...
**Example** | `{"systemOfRecords":"https://www.federalregister.gov/articles/2002/04/08/02-7376/..."}`
...
**Required** | Yes, if applicable
**Accepted Values** | ISO 8601 Date
**Usage Notes** | This field should contain an interval of time defined by the start and end dates for which the dataset is applicable. Dates should be formatted as pairs of {start datetime/end datetime} in the [ISO 8601](http://en.wikipedia.org/wiki/ISO_8601) format. Use a solidus ("/") to separate start and end times.
