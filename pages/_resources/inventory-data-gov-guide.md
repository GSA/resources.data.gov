...
*   **Part 1: About Inventory.data.gov** will help you understand what the tool is, its purpose, and its key features.
...
Inventory.data.gov is a specialized tool designed to help federal agencies create and manage the **legally required metadata** for their datasets. Its primary purpose is to generate **properly formatted JSON files** that comply with current metadata standards, fulfilling requirements under the **OPEN Government Data Act**.
...
This tool was designed by the Data.gov team (part of the U.S. General Services Administration, or GSA) to empower federal agencies in making their data discoverable and compliant.
...
**Behind the Scenes:**
Inventory.data.gov runs on a technology called CKAN (Comprehensive Knowledge Archive Network), which is an open-source platform widely used for government open data portals. It's hosted by GSA, just like the main Data.gov catalog.

**Important Note on Access:**
For most users, Inventory.data.gov is a secure, internal system. You'll need a user account to access and manage your agency's datasets. It's not publicly accessible, ensuring that only authorized individuals can make changes.

---

### 2. Key Features of Inventory.data.gov

Inventory.data.gov offers several powerful features to help you efficiently manage your agency's data and meet OPEN Government Data Act requirements:

*   **Easily Import Existing Data:** If your agency already has a `data.json` file or an enterprise data inventory, you can import that information directly into Inventory.data.gov, saving you time and effort. **[FLAG FOR FOLLOW-UP: Confirm if this is a direct user action or a process handled by the Product Manager.]**
*   **Flexible Agency Structure & Permissions:** Inventory.data.gov uses an "organization" structure to group datasets and manage user access. This allows for:
    *   Grouping all datasets for an entire agency.
    *   Separating datasets for individual bureaus or sub-agencies.
    *   Managing user permissions to ensure the right people can view, add, edit, or delete datasets for their specific area. **[FLAG FOR FOLLOW-UP: Rephrase to clarify that organizations are largely pre-established or automatically created, and user permissions are assigned by the Product Manager upon request, rather than users "setting up" organizations directly.]**
*   **Comprehensive Dataset Management:** This platform allows you to:
    *   **Create new** dataset entries.
    *   **Modify** existing dataset details.
    *   **Delete** datasets that are no longer needed.
    *   Manage both **public** (open) and **non-public** (restricted) datasets.
*   **Generate Compliance-Ready Data.json Files:** Export your validated datasets into a `data.json` file that meets the **DCAT-US standard**. This is the format Data.gov uses to "harvest" and display your data.
    *   The system provides an **error log** if any datasets don't meet the required standards, helping you quickly identify and fix issues before publishing.

---

### 3. What are "Organizations" in Inventory.data.gov?
...
**How Your Organization is Handled:**
When you request an account, we'll either link you to your agency's existing organization or, if one doesn't exist yet, we'll automatically create a new one for your agency. Your main task is simply to clearly state your agency during the account request process.

---

# Part 2: How to Use Inventory.data.gov

### 1. Getting Started: Creating Your User Account

Follow these steps to get your Inventory.data.gov account set up:

1.  **For New Users: Complete the New User / Validation Form.**
    ...
    *   **Information You'll Need to Provide:**
        *   **Your Name:** (Required)
        *   **Your Government Email:** (Required)
        *   **Your Phone Number:**
        *   **For which government department and/or agency do you need access to in inventory.data.gov?:** (Required - this is crucial for us to link you to the correct organization!)
        *   **Agreement to GSA IT Rules of Behavior:** You'll need to agree to the GSA IT Rules of Behavior (linked on the form: `https://www.gsa.gov/directives-library/gsa-information-technology-it-general-rules-of-behavior-4`). To confirm, type "I agree" in the designated box.
        *   **Date Confirmation:** In the box provided, please type today's date. This confirms your request for a new account.
        *   **Do you need a NEW account?:** Select "yes" for a new account request.
        *   **Questions? Comments? Want us to remove your account?:** Use this optional field for any additional notes.

    *   **If the online form is restricted:** Contact the Product Manager at **`datagovhelp@gsa.gov`** to request a Word version of the form.

2.  **Perform Your First Login via Login.gov (to establish your account).**
    This step allows Inventory.data.gov to recognize your identity. Please note, your `login.gov` account email **must be your .gov or .mil email address**.

    *   First, go to `login.gov` and follow their instructions to link your PIV/CAC card to your `login.gov` account. This ensures strong authentication.
    *   Once your `login.gov` account is ready, navigate to Inventory.data.gov.

    **💡 Important Tip for First-Time Logins:**
    When you arrive at Inventory.data.gov, look for a **small "Log In" button** usually located in the **top-right corner** of the screen. Click this to perform your initial login using your `login.gov` credentials.

3.  **Email the Product Manager to Activate Your Permissions.**
    *   **Crucial Step:** After you've completed your first login to Inventory.data.gov through `login.gov` (as described in Step 2), please email the Product Manager at **`datagovhelp@gsa.gov`**.
    *   **In your email, please clearly state your agency or sub-agency name** (e.g., "Department of Labor," "Farm Service Agency," etc.). This is how we'll confirm which organization to link you to, or to create if it's new.
    *   The Product Manager will then assign the necessary "Editor" permissions to your account and link you to your specified organization.
    *   **Heads Up:** When your access is granted, the Product Manager will typically include other existing members of your organization in the email notification. This helps keep everyone informed about new editors joining the team.

**What to Expect Next:**
Once your permissions are set, you're all good to go! For all future logins to Inventory.data.gov, you'll simply use your `login.gov` account and be able to manage your agency's datasets without any further assistance.

---

### 2. Navigating Inventory.data.gov and Finding Your Organization
...
3.  **Find Your Agency's Organization:**
...
2.  **General Tips for Entering Metadata:**
...
3.  **Entering Required Metadata (Step 1 of 3):**
...
4.  **Entering Additional Metadata (Step 2 of 3):**
...
5.  **Uploading Resources (Step 3 of 3):**
    ...
    *   **To recover:**
        1.  Log back into Inventory.data.gov.
        2.  Navigate to the **"My Datasets"** tab on your user dashboard.
        3.  Click on the title of the dataset you were working on to reopen it for editing.
        4.  Return to the "Resource Upload" section and re-attempt the upload of the resource file.

---

### 4. Understanding Dataset Status: Draft vs. Published
...
### 5. Exporting Your Agency's Data.json File
...
2.  **Choose Your Export Option:**
    ...
    *   **"Export data.json"**:
        *   Click this button to generate and download your agency's official `data.json` file. This file will contain all your **"Published"** datasets, formatted for Data.gov.
        *   *Note:* Clicking "Export data.json" will prompt you to download a file typically named `unredacted.zip`.
    *   **"Export Drafts"**:
        *   Clicking this button will export all datasets that are currently in **"Draft"** status to a separate file. This is useful for review purposes. When you click it, your browser will prompt you to download a file typically named `draft.zip`. These datasets will **not** be included in your official `data.json` for Data.gov.
...
4.  **Understand the Downloaded Zip File:**
...
5.  **CRITICAL: Check for Errors in `errorlog.txt`!**
...
6.  **Resolve Any Errors (If Needed):**
...
7.  **Post Your Data.json File:**
    *   Once you have a clean `data.json` file (without errors), you must **post it on your agency’s public website at `agency.gov/data.json`**. This is the location where the Data.gov catalog will "harvest" your metadata.

8.  **First-Time Harvest Setup (If Applicable):**
    *   If this is the very first time your agency is posting a `data.json` file at `agency.gov/data.json`, please contact the Product Manager at **`datagovhelp@gsa.gov`**. You'll need to coordinate with them to ensure Data.gov is set up to harvest your new source correctly.

---

### 6. Automating Data.json Downloads by Agencies

For agencies that prefer an automated approach, it's possible to use the Inventory.data.gov API and your unique API key to programmatically download your `data.json` file. This can streamline the process of updating your agency's website.

1.  **Obtain Your API Key:**
    *   Your API key is essential for authenticating your automated requests.
    *   You can find your API key at the bottom of the left sidebar of your user account page.
    *   To access your user account page, navigate to `https://inventory.data.gov/user/[your-username]` (replace `[your-username]` with your actual Inventory.data.gov username).

2.  **Make an Automated Request (Example using `httpie`):**
    You can use command-line tools like `httpie` (a `curl`-like tool) to make the request. Here's what it would look like for downloading your **Unredacted Inventory (EDI)**:

    ```bash
    http --download https://inventory.data.gov/organization/[organization-id]/unredacted.json X-CKAN-API-Key:xxx-your-api-key-xxx
    ```
    *   **Replace `[organization-id]`**: Use the unique identifier for your agency's organization in Inventory.data.gov.
    *   **Replace `xxx-your-api-key-xxx`**: Use your actual API key that you obtained in Step 1.

3.  **Review for Errors Before Full Automation:**
    *   The process above downloads a `.zip` file containing the `data.json` and, if any issues were found, an `errorlog.txt`.
    *   **Crucial Step:** It is strongly recommended that you **manually review and upload your `data.json` file a few times** before setting up a fully automated process.
    *   **Why?** This ensures that you can identify and correct any errors (by checking for the `errorlog.txt` file) that might be present. Automating before resolving errors could inadvertently exclude datasets from your `data.json
