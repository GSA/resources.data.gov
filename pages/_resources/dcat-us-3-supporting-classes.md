...
details: >+
  ...
  **Standard** is used in `conformsTo` on Dataset, Distribution, DataService, and Catalog. A Standard declares what specification or schema a resource follows. It should include a `title` and an `identifier` (a stable URI for the standard). A single resource can conform to multiple 
  standards -- for example, a geospatial dataset might conform to both DCAT-US 3.0 and ISO 19115.
  ...
  **CUIRestriction** applies specifically to distributions that contain Controlled Unclassified Information. It requires a `cuiBannerMarking` (the CUI banner marking string from the NARA CUI Registry, such as `CUI//SP-PRVCY`) and a `designationIndicator` identifying the agency that designated the information as CUI. Distributions that do not contain CUI should set `cuiRestriction` to `null`.
  ...
  | Agent | `publisher` on Catalog, DataService; `creator`, `contributor` on Dataset | `name` | [Agent class](https://resources.data.gov/standards/catalog/dcat-us-3/agents/#class-agent-) |
  
  | Organization | `publisher` on Dataset; `subOrganizationOf` nesting | `name` | [Organization class](https://resources.data.gov/standards/catalog/dcat-us-3/agents/#class-organization-) |
  
  | Kind | `contactPoint` on all primary classes | `fn`, `hasEmail` (with mailto: prefix) | [Kind class](https://resources.data.gov/standards/catalog/dcat-us-3/agents/#class-kind-) |
  
  | Location | `spatial` on Dataset, DataService, DatasetSeries, Catalog | none (include `prefLabel` at minimum) | [Location class](https://resources.data.gov/standards/catalog/dcat-us-3/temporal-spatial-metrics/#class-location-) |
  
  | PeriodOfTime | `temporal` on Dataset, DataService, DatasetSeries, Catalog | `startDate` or `endDate` (at least one) | [PeriodOfTime class](https://resources.data.gov/standards/catalog/dcat-us-3/temporal-spatial-metrics/#class-periodoftime-) |
  
  | Concept | `theme`, `category`, `status`, restriction fields, and others | `prefLabel` (if object form) | [Concept class](https://resources.data.gov/standards/catalog/dcat-us-3/identifiers-and-relationships/#class-concept-) |
  
  | ConceptScheme | `themeTaxonomy` on Catalog; `inScheme` on Concept | `title` | [ConceptScheme class](https://resources.data.gov/standards/catalog/dcat-us-3/identifiers-and-relationships/#class-conceptscheme-) |
  
  | Document | `page`, `landingPage`, `homepage` | `title` | [Document class](https://resources.data.gov/standards/catalog/dcat-us-3/quality-governance/#class-document-) |
  
  | Standard | `conformsTo` on all primary classes | `title` recommended | [Standard class](https://resources.data.gov/standards/catalog/dcat-us-3/quality-governance/#class-standard-) |
  
  | Identifier | `identifier`, `otherIdentifier` | none (plain string or structured object) | [Identifier class](https://resources.data.gov/standards/catalog/dcat-us-3/identifiers-and-relationships/#class-identifier-) |
  
  | Checksum | `checksum` on Distribution | `algorithm`, `checksumValue` | [Checksum class](https://resources.data.gov/standards/catalog/dcat-us-3/identifiers-and-relationships/#class-checksum-) |
  
  | QualityMeasurement | `hasQualityMeasurement` on Dataset, Distribution, DataService | `value`, `isMeasurementOf` | [QualityMeasurement class](https://resources.data.gov/standards/catalog/dcat-us-3/temporal-spatial-metrics/#class-qualitymeasurement-) |
  
  | Metric | inside QualityMeasurement | `expectedDataType`, `inDimension` | [Metric class](https://resources.data.gov/standards/catalog/dcat-us-3/temporal-spatial-metrics/#class-metric-) |
  
  | Activity | `wasGeneratedBy`, `wasUsedBy` on Dataset, DataService | `label` | [Activity class](https://resources.data.gov/standards/catalog/dcat-us-3/temporal-spatial-metrics/#class-activity-) |
  
  | Attribution | `qualifiedAttribution` on Dataset, Catalog, DataService | `hadRole`, `agent` | [Attribution class](https://resources.data.gov/standards/catalog/dcat-us-3/quality-governance/#class-attribution-) |
  
  | Relationship | `qualifiedRelation` on Dataset | `hadRole`, `relation` | [Relationship class](https://resources.data.gov/standards/catalog/dcat-us-3/identifiers-and-relationships/#class-relationship-) |
  
  | CatalogRecord | `record` on Catalog | `modified`, `primaryTopic` | [CatalogRecord class](https://resources.data.gov/standards/catalog/dcat-us-3/quality-governance/#class-catalogrecord-) |
  
  | AccessRestriction | `accessRestriction` on Distribution | `restrictionStatus` | [AccessRestriction class](https://resources.data.gov/standards/catalog/dcat-us-3/constraints-and-restrictions/#class-accessrestriction-) |
  
  | UseRestriction | `useRestriction` on Distribution | `restrictionStatus` | [UseRestriction class](https://resources.data.gov/standards/catalog/dcat-us-3/constraints-and-restrictions/#class-userestriction-) |
  
  | CUIRestriction | `cuiRestriction` on Distribution | `cuiBannerMarking`, `designationIndicator` | [CUIRestriction class](https://resources.data.gov/standards/catalog/dcat-us-3/constraints-and-restrictions/#class-cuirestriction-) |
