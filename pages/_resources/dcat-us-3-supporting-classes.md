...
details: >+
  ...
  For full technical specifications and field-level details, see the [DCAT-US 3.0 technical schema reference](../../../standards/catalog/dcat-us-3/).
  ...
  **Kind** is the contact information class, modeled on vCard. Every `contactPoint` field uses Kind. It requires a formatted name (`fn`) and an email address (`hasEmail`) formatted with a `mailto:` prefix. You can optionally add a phone number, position title, and organization name.
  ...
  [Full technical reference: Agents](../../../standards/catalog/dcat-us-3/agents/)
  ...
  **PeriodOfTime** is used in any `temporal` field. It requires at least one of `startDate` or `endDate` -- both are not required. Open-ended periods are valid, so a dataset that is ongoing can have a `startDate` with no `endDate`. All date values accept full dates (`YYYY-MM-DD`), datetimes, year-month (`YYYY-MM`), or year-only (`YYYY`) format.
  ...
  [Full technical reference: Temporal, spatial, and metrics](../../../standards/catalog/dcat-us-3/temporal-spatial-metrics/)
  ...
  **ConceptScheme** represents the vocabulary or taxonomy that a set of Concepts belongs to. It is used in `themeTaxonomy` on Catalog to declare what controlled vocabulary your theme values come from. A ConceptScheme requires a `title`. You can also link it to a version, publication date, and description. If your agency uses the Data.gov topic categories, ISO 19115 topic categories, or your own internal taxonomy, you would declare that here.
  ...
  [Full technical reference: Constraints and controlled vocabularies](../../../standards/catalog/dcat-us-3/constraints-and-restrictions/)
  ...
  **Standard** is used in `conformsTo` on Dataset, Distribution, DataService, and Catalog. A Standard declares what specification or schema a resource follows. It should include a `title` and an `identifier` (a stable URI for the standard). A single resource can conform to multiple standards -- for example, a geospatial dataset might conform to both DCAT-US 3.0 and ISO 19115.
  ...
  [Full technical reference: Identifiers and relationships](../../../standards/catalog/dcat-us-3/identifiers-and-relationships/)
  ...
  **Activity** is used in `wasGeneratedBy` and `wasUsedBy` on Dataset and DataService. It describes a process or event that created or used the resource -- for example, a data collection exercise, a quality control review, or a modeling run. An Activity requires only a `label`.
  ...
  [Full technical reference: Quality and governance](../../../standards/catalog/dcat-us-3/quality-governance/)
  ...
  **CUIRestriction** applies specifically to distributions that contain Controlled Unclassified Information. It requires a `cuiBannerMarking` (the CUI banner marking string from the NARA CUI Registry, such as `CUI//SP-PRVCY`) and a `designationIndicator` identifying the agency that designated the information as CUI. Distributions that do not contain CUI should set `cuiRestriction` to `null`.
  ...
  [Full technical reference: Constraints and restrictions](../../../standards/catalog/dcat-us-3/constraints-and-restrictions/)
  
  ---
  
  ### Quick reference
  
  | Class | Where you use it | Requires |
  |---|---|---|
  | Agent | `publisher` on Catalog, DataService; `creator`, `contributor` on Dataset | `name` |
  | Organization | `publisher` on Dataset; `subOrganizationOf` nesting | `name` |
  | Kind | `contactPoint` on all primary classes | `fn`, `hasEmail` (with mailto: prefix) |
  | Location | `spatial` on Dataset, DataService, DatasetSeries, Catalog | none (include `prefLabel` at minimum) |
  | PeriodOfTime | `temporal` on Dataset, DataService, DatasetSeries, Catalog | `startDate` or `endDate` (at least one) |
  | Concept | `theme`, `category`, `status`, restriction fields, and others | `prefLabel` (if object form) |
  | ConceptScheme | `themeTaxonomy` on Catalog; `inScheme` on Concept | `title` |
  | Document | `page`, `landingPage`, `homepage` | `title` |
  | Standard | `conformsTo` on all primary classes | `title` recommended |
  | Identifier | `identifier`, `otherIdentifier` | none (plain string or structured object) |
  | Checksum | `checksum` on Distribution | `algorithm`, `checksumValue` |
  | QualityMeasurement | `hasQualityMeasurement` on Dataset, Distribution, DataService | `value`, `isMeasurementOf` |
  | Metric | inside QualityMeasurement | `expectedDataType`, `inDimension` |
  | Activity | `wasGeneratedBy`, `wasUsedBy` on Dataset, DataService | `label` |
  | Attribution | `qualifiedAttribution` on Dataset, Catalog, DataService | `hadRole`, `agent` |
  | Relationship | `qualifiedRelation` on Dataset | `hadRole`, `relation` |
  | CatalogRecord | `record` on Catalog | `modified`, `primaryTopic` |
  | AccessRestriction | `accessRestriction` on Distribution | `restrictionStatus` |
  | UseRestriction | `useRestriction` on Distribution | `restrictionStatus` |
  | CUIRestriction | `cuiRestriction` on Distribution | `cuiBannerMarking`, `designationIndicator` |
